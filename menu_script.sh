...
# Commit 20210403 (Gerardo Guerrero)
# - Added Exiftool to the end of all Metagoofil commands. 

# Commit 20210401 (Gerardo Guerrero)
# - Fixed Fierce command. Changeed "fierce -dns" > "fierce --domain".

# Commit 20210129 (Gerardo Guerrero)
# - Removed Nikto update. Added manual update. 
# - Fixed Fierce DNS command. Changed (-dns) to (--domain)
# - Added exiftool at the end of Metagoofil only search.

# Commit 20200207 (Steven Palmesano):
# - Fixed the metagoofil command. Metadata extraction will need to be done with exiftool. Have to add that later.

# Commit 20200128 (Steven Palmesano):
# - The script now copies targets.txt to the client directory after the client name is entered.
# - Removed firewalk from option 14. It still remains in 3, 4, and 5.
# - Moved mkdir commands for firewalk, joomla, and hydra to their respective places instead of right at the start.
# - Updated nikto command to use both port 80 and 443.

# Commit 20200109 (Steven Palmesano):
# - Changed "theharvester" to "theHarvest". No idea why the devs thought this was a good idea.

# Commit 20190408 (Steven Palmesano):
# - Fixed an issue with the WPScan when a redirect is required (--ignore-main-redirect).

# Commit 20181219 (Steven Palmesano):
# - Fixed an issue with the WPScan commands (--update was missing).

# Commit 20181211 (Steven Palmesano):
# - Fixed an issue with the WPScan commands.

# Commit 20181210 (Steven Palmesano):
# - Fixed a naming issue with option 9 (FierceDNS).
# - Added the hackertarget reverse IP lookup to the all-in-one scans.
# - Changed any instance of /root/ to ~/ in case the command is run as a user who isn't root (for testing).

# Commit 20181203 (Steven Palmesano):
# - Updated WPScan command for v3.
# - Added WPScan to the all-in-one scans.

# Commit 20180914 (Steven Palmesano):
# - Changed options 1, 2, and 14 (nmap) to reduce the amount of scans.

# Commit 20180703 (Steven Palmesano):
# - Fixed the output progress with option 15.

# Commit 20180701 (Steven Palmesano):
# - Fixed the output of the current progress so it clears the last status before printing on the line. Did this with \033[K
# - Fixed the WPScan command.
# - Automated the date being added to the output.
